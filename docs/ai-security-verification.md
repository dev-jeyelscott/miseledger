...
| Sensitive observability | `ai.run.*` logs carry only lifecycle IDs, provider, status, safe error code, and token counters. Tool-call metadata is allowlisted and excludes prompts, responses, arguments, result bodies, profile paths, and credentials. | `app/Support/Ai/AiObservability.php`, `app/Actions/Ai/RecordAiToolCall.php`, `tests/Feature/AiSecurityHardeningTest.php` |
