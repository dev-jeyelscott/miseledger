...
```bash
XDEBUG_MODE=debug docker compose up -d --force-recreate webapp
```

Disable it again:

```bash
XDEBUG_MODE=off docker compose up -d --force-recreate webapp
```
