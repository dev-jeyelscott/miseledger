...
test('draft transfer creation is serialized against a concurrent inventory-item deactivation', function () {
    ...
    $resultPath = tempnam(sys_get_temp_dir(), 'miseledger-item-deactivation-');
        ...
        try {
            DB::transaction(function (): void {
                $lockedItem = InventoryItem::query()
                    ->whereKey($this->inventoryItem->id)
                    ->lockForUpdate()
                    ->firstOrFail();
                ...
                $lockedItem->update(['active' => false]);
            });
    ...
    // Give the child a head start so it acquires the inventory-item row lock
    ...
    try {
        ...
        expect(pcntl_wifexited($status))->toBeTrue()
            ...
            ->and($thrown->errors())->toHaveKey('lines.0.inventory_item_id')
            ->and($this->inventoryItem->refresh()->active)->toBeFalse()
            ->and(
                StockTransfer::query()
                    ->where('organization_id', $this->organization->id)
                    ->where('number', 'TR-CONCURRENCY-002')
                    ->exists(),
            )->toBeFalse();
    } finally {
        if (! $childReaped) {
...
test('inventory-item deactivation is serialized against a concurrent draft transfer creation', function () {
    if (DB::connection()->getDriverName() !== 'pgsql') {
        $this->markTestSkipped('PostgreSQL concurrency coverage runs in CI.');
    }
    ...
    $resultPath = tempnam(sys_get_temp_dir(), 'miseledger-item-transfer-first-');
    ...
    // commits the draft transfer before this process attempts to
    // deactivate the item.
    usleep(300000);
