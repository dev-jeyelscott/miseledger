...
use App\Http\Requests\Inventory\RecordOpeningBalanceRequest;
...
{
    /**
     * Show the explicit initial-stock workflow.
     */
    ...
    /**
     * Record one auditable cost-bearing opening movement.
     */
    public function store(
        ...
        $movement = $recordOpeningBalance->handle(
            ...
            idempotencyKey: 'opening_balance:manual:'
                .(string) $validated['operation_id'],
            actor: $actor,
            notes: isset($validated['notes'])
                && is_string($validated['notes'])
                    ? $validated['notes']
                    : null,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => __(
                'Opening balance movement #:id recorded for :item.',
                [
                    'id' => $movement->id,
    ...
    /**
     * Build active tenant-owned opening-balance selections.
     *
